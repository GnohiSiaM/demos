...
var isUsernameNonexistent = true;
...
//	check(username,"长度须为3-20位且无空格",blurOrSubmit,function(val){
...
//	});
	...
	checkUsername(username,"长度3-20位且无特殊字符",blurOrSubmit,function(val){
...
//	});
	...
	return (isUsernameNonexistent && status);
}//#end  validation();
	...
	obj.onblur = function(){
		...
		if(repass.value != ''){
			check(repass,"请保持两次密码一致","Submit",function(val){
				if(val.match(/^\S+$/) && val.length>=6 && val.length<=30 && val==obj.value){
					return true;
				}else{
					return false;
				}
...
//	var username = document.getElementById("username");
//	var tips = getTips(username);
//	
//	var myXHRObject = getXMLHttpObject();
//	if(myXHRObject){		//XMLHttpRequest对象创建成功才执行后续代码
//		//第一个参数：请求的方式 GET/POST
//		//第二个参数：指定url，对哪个页面发出ajax请求（本质仍是http请求）
//		//第三个参数：true表示使用异步机制，false表示不使用异步机制
//		var url="RegisterServlet?userName="+username.value ;
//		//打开请求
//		myXHRObject.open("get",url,true);
//		//发送请求，如果是get请求send()内填null即可;如果是post请求，则填入实际的数据
//		myXHRObject.send(null);
//		myXHRObject.onreadystatechange = function(){
//			if(myXHRObject.readyState==4 && myXHRObject.status==200){
//				if(myXHRObject.responseText==1){
//					isUsernameNonexistent = false;
//					tips.className = 'status3';
//					tips.innerHTML = '用户已存在,请更名或直接<a href="login.jsp">登录</a>';
//				}else{
//					isUsernameNonexistent = true;
//				}
//			}
//		}
//	}
...
function checkUsername(obj,info,blurOrSubmit,func){
	var tips = getTips(obj);
	var myXHRObject = getXMLHttpObject();
	var url = "";
	var isUsernameAvailable = null;
	obj.onfocus = function(){
		tips.className = "tips status2";
		tips.innerHTML = info;
	}
	obj.onblur = function () {
	    ...
	    isUsernameAvailable = func(obj.value);
	    if (myXHRObject) {
	        url = "Register.aspx?userName=" + obj.value;
	        myXHRObject.open("get", url, true);
	        myXHRObject.setRequestHeader("If-Modified-Since", "0"); //解决IE下Ajax缓存问题
	        myXHRObject.send();
	        //url = "Register.aspx";
	        //myXHRObject.open("post", url, true);
	        //myXHRObject.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
	        //myXHRObject.send("userName=" + obj.value);
	        myXHRObject.onreadystatechange = function () {
	            if (myXHRObject.readyState == 4 && myXHRObject.status == 200) {
	                //alert(myXHRObject.responseText);
	                if (myXHRObject.responseText == 1) {
	                    isUsernameNonexistent = false;
	                    tips.className = 'tips status3';
	                    tips.innerHTML = '用户已存在,请更名或直接<a href="Login.aspx?userName=' + obj.value + '" style="color:#3587F0;text-decoration:none;">登录</a>';
	                } else {
	                    isUsernameNonexistent = true;
	                    if (isUsernameAvailable) {
	                        ...
	                        tips.className = "tips status3";
	                        if (obj.value == '') tips.innerHTML = "该项不能为空";
	                        else tips.innerHTML = info;
	                    }
	                }
	            }
	        }
